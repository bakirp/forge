...
.forge/
  ...
  review/
    [feature-name].md               <- /review (feature-named; falls back to report.md)
    adversarial.md                  <- /review adversarial
  verify/
    [feature-name].md               <- /verify (feature-named; falls back to report.md)
    screenshots/
      [test-name].png               <- /verify (on failure)
  debug/
    [feature-name].md               <- /debug (feature-named; falls back to report.md)
  browse/
    [feature-name].md               <- /browse (feature-named; falls back to report.md)
    screenshots/                    <- /browse
  design/
  ...
  benchmark/
    [feature-name].md               <- /benchmark (feature-named; falls back to report.md)
  build/
    [feature-name].md               <- /build Step 6.5 (feature-named handoff artifact; falls back to report.md)
  context/
  ...
  compliance.jsonl                    <- scripts/compliance-log.sh
  autopilot/
...
**Path:** `.forge/review/[feature-name].md` (falls back to `.forge/review/report.md`)
...
**Path:** `.forge/verify/[feature-name].md` (falls back to `.forge/verify/report.md`)
...
**Path:** `.forge/debug/[feature-name].md` (falls back to `.forge/debug/report.md`)
...
**Path:** `.forge/build/[feature-name].md` (falls back to `.forge/build/report.md`)
...
```json
{
  ...
  "artifacts": {
    ...
    "build": ".forge/build/add-user-auth.md",
    "review": ".forge/review/add-user-auth.md",
    "verify": ".forge/verify/add-user-auth.md",
    "debug": ".forge/debug/add-user-auth.md"
  },
  "blockers": [],
  "history": [
...
| `feature_name` | string | Slugified feature name generated by `/think` Step 4.5. Stored via `scripts/manifest.sh feature-name`. Falls back to `"report"` if not set. |
...
**Produced by:** `scripts/compliance-log.sh`
**Consumed by:** `/ship` (audit trail), human review
**Path:** `.forge/compliance.jsonl`

Each line is a JSON object recording a rule violation detected during pipeline execution:

```json
{"timestamp":"2026-04-12T10:30:00Z","phase":"build","rule":"coverage-threshold","detail":"Line coverage 62% below threshold 80%","severity":"major","file":"src/auth.ts"}
```
...
|-------|------|----------|-------------|
| `timestamp` | string | Yes | ISO 8601 timestamp of the violation |
| `phase` | string | Yes | Pipeline phase that detected the violation (`build`, `review`, `ship`, `verify`, `autopilot`) |
| `rule` | string | Yes | Machine-readable rule identifier (e.g., `coverage-threshold`, `stale-artifact`, `missing-test`) |
| `detail` | string | Yes | Human-readable description of the violation |
| `severity` | string | Yes | `critical`, `major`, `minor` |
| `file` | string | No | File path related to the violation, if applicable |

Integrated into `/build`, `/review`, `/ship`, `/verify`, and `/autopilot`. Each skill calls `scripts/compliance-log.sh` when a rule violation is detected. The log is append-only and cumulative across pipeline runs.

---

## Feature-Named Artifacts

Handover documents are named after the feature rather than using a generic `report.md`. For example, a task "Add user authentication" produces `.forge/build/add-user-auth.md` instead of `.forge/build/report.md`.

**How it works:**

1. `/think` Step 4.5 generates a slugified feature name from the task description.
2. The feature name is stored in the run manifest via `scripts/manifest.sh feature-name`.
3. All skills resolve the feature name via `scripts/manifest.sh resolve-feature-name`.
4. If no feature name is set (e.g., older runs or direct skill invocation), resolution falls back to `"report"` for backward compatibility.

This enables multiple feature artifacts to coexist under the same `.forge/` directory without overwriting each other.
...
| `/build` | architecture doc | source code, tests, compliance log | architecture doc |
| `/review` | architecture doc, source code | review report, compliance log | -- |
| `/review adversarial` | architecture doc, build report, source code | adversarial review report | -- |
| `/verify` | architecture doc | verify report, screenshots, compliance log | -- |
...
| `/ship` | review report, verify report, compliance log | release summary, PR, compliance log | review report, verify report |
