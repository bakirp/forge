...
var DEFAULTS = Object.freeze({
  ...
  lockTimeoutMs: 1000,
  idleInTransactionSessionTimeoutMs: 1e4,
  applicationName: "forge-pg-agent"
});
var ALLOWED_TOP_LEVEL = new Set(["SELECT", "WITH", "SHOW", "EXPLAIN"]);
var IDENTIFIER_RE = /[A-Za-z_]/;
var IDENTIFIER_PART_RE = /[A-Za-z0-9_$]/;
var STANDARD_PG_ENV_KEYS = [
  "PGHOST",
  "PGHOSTADDR",
  "PGPORT",
  "PGUSER",
...
class PgAgentError extends Error {
...
function hasStandardPgEnv(env = process.env) {
  return STANDARD_PG_ENV_KEYS.some((key) => {
    const value = env[key];
    return typeof value === "string" && value.length > 0;
  });
}
function resolveConnectionConfig(options = {}, env = process.env) {
  const connectionString = options.url ?? env.PG_URL;
  if (!connectionString && !hasStandardPgEnv(env)) {
    throw new PgAgentError("Provide --url, PG_URL, or standard PG* environment variables.", EXIT_CODES.INVALID_USAGE, "INVALID_CONNECTION_CONFIG");
  }
  const config = {};
  if (connectionString) {
...
function parseCliArgs(argv, env = process.env) {
  ...
  if (!["inspect", "describe", "query"].includes(command)) {
    throw new PgAgentError(`Unknown command: ${command}`, EXIT_CODES.INVALID_USAGE, "UNKNOWN_COMMAND", { command });
  }
  const options = {
    ...
    idleInTransactionSessionTimeoutMs: DEFAULTS.idleInTransactionSessionTimeoutMs
  };
  while (args.length > 0) {
    ...
    if (!token.startsWith("--")) {
      throw new PgAgentError(`Unexpected argument: ${token}`, EXIT_CODES.INVALID_USAGE, "UNEXPECTED_ARGUMENT", { token });
    }
    ...
    switch (key) {
      ...
      case "statement-timeout":
        options.statementTimeoutMs = parsePositiveInteger(requireValue(args, token), token);
        break;
      case "lock-timeout":
        options.lockTimeoutMs = parsePositiveInteger(requireValue(args, token), token);
        break;
      case "idle-timeout":
        options.idleInTransactionSessionTimeoutMs = parsePositiveInteger(requireValue(args, token), token);
        break;
      default:
        throw new PgAgentError(`Unknown option: ${token}`, EXIT_CODES.INVALID_USAGE, "UNKNOWN_OPTION", { option: token });
    }
  }
...
function validateCommandOptions(command, options) {
  if (command === "describe" && !options.table) {
    throw new PgAgentError("describe requires --table <name>.", EXIT_CODES.INVALID_USAGE, "MISSING_TABLE");
  }
  if (command === "query" && !options.sql) {
    throw new PgAgentError('query requires --sql "<statement>".', EXIT_CODES.INVALID_USAGE, "MISSING_SQL");
  }
  if (command !== "query") {
    for (const disallowed of [
      ...
      if (disallowed in options && options[disallowed] !== undefined && options[disallowed] !== DEFAULTS[keyForDefault(disallowed)]) {
        throw new PgAgentError(`${command} does not accept ${toFlagName(disallowed)}.`, EXIT_CODES.INVALID_USAGE, "INVALID_OPTION_FOR_COMMAND", { command, option: toFlagName(disallowed) });
      }
    }
  }
  if (command !== "describe" && options.table) {
    throw new PgAgentError(`${command} does not accept --table.`, EXIT_CODES.INVALID_USAGE, "INVALID_OPTION_FOR_COMMAND", { command, option: "--table" });
  }
}
...
function requireValue(args, flag) {
  ...
  if (!value) {
    throw new PgAgentError(`${flag} requires a value.`, EXIT_CODES.INVALID_USAGE, "MISSING_OPTION_VALUE", { option: flag });
  }
  return value;
}
function parsePositiveInteger(value, flag) {
  ...
  if (!Number.isSafeInteger(parsed) || parsed <= 0) {
    throw new PgAgentError(`${flag} expects a positive integer.`, EXIT_CODES.INVALID_USAGE, "INVALID_INTEGER", { option: flag, value });
  }
  return parsed;
}
...
async function inspectSchema(client, schema = DEFAULTS.schema) {
...
async function describeTable(client, schema = DEFAULTS.schema, table) {
  const result = await client.query(`
      SELECT
        column_name,
        CASE
...
async function executeReadOnlyQuery(client, sql, options = {}) {
  const validation = validateReadOnlySql(sql);
  if (!validation.ok) {
    throw new PgAgentError(validation.message, EXIT_CODES.UNSAFE_SQL, validation.errorCode, validation.analysis ? { analysis: validation.analysis } : undefined);
  }
  ...
  try {
    ...
    await setLocalConfig(client, "statement_timeout", String(options.statementTimeoutMs ?? DEFAULTS.statementTimeoutMs));
    await setLocalConfig(client, "lock_timeout", String(options.lockTimeoutMs ?? DEFAULTS.lockTimeoutMs));
    await setLocalConfig(client, "idle_in_transaction_session_timeout", String(options.idleInTransactionSessionTimeoutMs ?? DEFAULTS.idleInTransactionSessionTimeoutMs));
    const result = await client.query(sql);
    const elapsedMs = Math.round(performance.now() - startedAt);
    return formatQueryResult(result, options.maxRows ?? DEFAULTS.maxRows, elapsedMs);
  } finally {
    if (beganTransaction) {
...
async function connectClient(connectionConfig) {
  const client = new Client2(connectionConfig);
  try {
    await client.connect();
    return client;
  } catch (error) {
    await client.end().catch(() => {});
    throw new PgAgentError(error.message, EXIT_CODES.CONNECTION_FAILED, "CONNECTION_FAILED", { cause: error.code ?? error.name });
  }
}
async function runCommand(parsed) {
  ...
  try {
    switch (parsed.command) {
      ...
      case "describe":
        return await describeTable(client, parsed.options.schema, parsed.options.table);
      case "query":
        return await executeReadOnlyQuery(client, parsed.options.sql, parsed.options);
      default:
        throw new PgAgentError(`Unsupported command: ${parsed.command}`, EXIT_CODES.INVALID_USAGE, "UNKNOWN_COMMAND");
    }
  } catch (error) {
    if (error instanceof PgAgentError) {
      throw error;
    }
    throw new PgAgentError(error.message, EXIT_CODES.QUERY_FAILED, "QUERY_FAILED", { cause: error.code ?? error.name });
  } finally {
    await client.end().catch(() => {});
  }
}
...
async function runCli(argv, env = process.env, io = process) {
  try {
